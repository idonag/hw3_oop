...
import BusinessLayer.Tiles.Enemies.Boss;
import BusinessLayer.Tiles.Players.*;
...
public class TileFactory {
    ...
    public Enemy produceEnemy(char tile, Position position) {
        ...
        Enemy e =  initEnemies().get(tile).get();
    ...
    public Player producePlayer(int idx){
		...
		Player p = initPlayers().get(idx).get();

        return p;
    }
    public void presentPlayer(){
            messageCallBack.send("Choose a Player"+"\n");
            int choose = 0;
            String message= "";
            for (Supplier<Player> playerSupplier : initPlayers()) {
                message+=choose +" "+playerSupplier.get().describe()+"\n";
                choose ++;
            }
